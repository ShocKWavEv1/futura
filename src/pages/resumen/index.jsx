...
const Resumen = () => {
  ...
  const renderDynamicDates = (firstDate, secondDate) => {
    if(secondDate === undefined) {
        return `el dia *${firstDate?.getDate()}-${firstDate?.getMonth() + 1}-${firstDate?.getFullYear()}*`
    } else {
        return `del dia *${firstDate?.getDate()}-${firstDate?.getMonth() + 1}-${firstDate?.getFullYear()}* al dia *${secondDate?.getDate()}-${secondDate?.getMonth() + 1}-${secondDate?.getFullYear()}*`
    }
}

  const handleList = () => {
    let stringData = shoppingCart.reduce(
      (result, item, idx) => {
    ...
    }* estoy interesado en rentar equipo con ustedes ${renderDynamicDates(firstDate, secondDate)} para el proyecto: *${
    ...
    >
      ...
      >
        <MotionAnimation delay={0}>
          ...
          >
            Los siguientes campos no son requeridos, pero nos ayudaria mucho
            para poder darte un mejor estimado
          </Text>
        </MotionAnimation>
        <Box w="100%" p="40px 0px">
          <Stack direction="column" spacing="40px">
            <Box w="auto" display="flex" flexDirection="column">
              ...
              <Box pt="10px" w="380px">
                <Input h="50px" placeholder="Escrbie tu nombre" onChange={(e) => setAlias(e.target.value)} />
              </Box>
            </Box>
            <Box w="auto" display="flex" flexDirection="column">
              <Text
              ...
              <Box pt="10px" w="380px">
                <Input h="50px" placeholder="Escrbie tu nombre" onChange={(e) => setNameProject(e.target.value)} />
              </Box>
            </Box>
            <Box w="auto" display="flex" flexDirection="column">
              <Text
